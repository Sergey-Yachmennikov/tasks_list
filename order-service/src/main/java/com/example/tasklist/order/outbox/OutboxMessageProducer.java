 ...
 * <p>
 * Поэтому консьюмеры {@code order-created} обязаны быть идемпотентными (дедуп по orderId),
 * поскольку падение между {@link KafkaProducer#send} и {@link #markSent} может привести к
 * повторной доставке одного и того же сообщения.
 */
@Component
public class OutboxMessageProducer {
    ...
    private void publish(OutboxMessage message) {
        ...
        MDC.put("aggregateId", message.aggregateId().toString());
        try {
